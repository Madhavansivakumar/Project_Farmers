...
const AddDemand = ({ onAddDemand }) => {
    ...
    const onSubmit = async (e) => {
        ...
        if (token) {
            ...
            try {
                const res = await axios.post('http://localhost:5000/api/demands', formData);
                onAddDemand(res.data);
                setFormData({ productType: '', quantityNeeded: '', deliveryDate: '' });
